...
def test_get_course_by_id_exists():
    response = client.get("/courses/665e50d5f6ebfc82bd0fdd53")
    assert response.status_code == 200
    course = response.json()
    # get the course from the database
    course_db = db.courses.find_one({'_id': ObjectId('665e50d5f6ebfc82bd0fdd53')})
...
#Test Get Course Chapter Info
def test_get_chapter_info():
    response = client.get("/courses/665e50d5f6ebfc82bd0fdd53/1")
    assert response.status_code == 200
...
def test_get_chapter_info_not_exists():
    response = client.get("/courses/665e50d5f6ebfc82bd0fdd53/990")
    assert response.status_code == 404
    assert response.json() == {'detail': 'Chapter not found'}
    
# Test Post Course Rating
def test_rate_chapter():
    course_id = "665e50d5f6ebfc82bd0fdd53"
    chapter_id = "1"
    rating = 1
...
def test_rate_chapter_not_exists():
    response = client.post("/courses/665e50d5f6ebfc82bd0fdd53/990/rate", json={"rating": 1})
    assert response.status_code == 404
